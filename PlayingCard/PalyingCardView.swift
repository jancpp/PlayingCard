...
class PlayingCardView: UIView
{
    ...
    private func createCornerLabel() -> UILabel {
        let label = UILabel()
        label.numberOfLines = 0 // zero means use as many lines as you need
        addSubview(label)
        return label
    }
    ...
    private func configureCornerLabel(_ label: UILabel) {
        ...
        label.frame.size = CGSize.zero // clears out size, so it can expand both, hight and width, on the next line (not just width)
        label.sizeToFit()
